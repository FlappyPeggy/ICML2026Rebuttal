...
class CLIPCAM:
  ...
  def __call__(self, ori_img, text, scale=1.0, fg_mask=None, text_feature=None):
    ...
    if text_feature is None:
      fg_features_temp = zeroshot_classifier(
          text, None, self.clip_model, self.device
      ).to(self.device)
    else:
      fg_features_temp = text_feature.to(self.device)
  ...
  def forward(self, ori_img, text_features, scale=1.0, fg_mask=None):
      ...
      with torch.no_grad():
          ...
          trans_mat = torch.matmul(trans_mat, trans_mat)
          trans_mat = trans_mat.unsqueeze(0) * aff_mask.unsqueeze(1) 

          cam_flat = cams.view(C, -1, 1)                           
          cam_token = torch.matmul(trans_mat, cam_flat).squeeze(-1) 
